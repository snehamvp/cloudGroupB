...
echo "Zero OSD down Lookup $(($stop_time-$start_time)) ms" >> /root/CloudGroupB_Git_Update/cloudGroupB/ceph_scr/faultRecovery.xls
...
#echo "Zero OSD down Read time $(($stop_time-$start_time)) ms" >> /root/CloudGroupB_Git_Update/cloudGroupB/ceph_scr/faultRecovery.xls
...
#Evaluating the fault recovery time
#echo "Read time: $(($stop_time-$start_time)) ms"
#echo "1 OSD down Read time $(($stop_time-$start_time)) ms" >> /root/CloudGroupB_Git_Update/cloudGroupB/ceph_scr/faultRecovery.xls
...
rm -rf randomfile1
dd if=/dev/urandom of=randomfile1 bs=4M count=375 conv=fdatasync
...
echo "Sadha time $(($stop_time-$start_time)) ms"
...
echo "recover time $(($stop_time-$start_time)) ms"


#start_time=$(date -u +"%s%3N")
#ceph osd map test_rados randomfile1
#rados -p test_rados get randomfile1 test2.out
#top_time=$(date -u +"%s%3N")
...
#rados -p test_rados get randomfile1 test1.out
#stop_time=$(date -u +"%s%3N")
#echo "2 OSD down Read time $(($stop_time-$start_time)) ms" >> /root/CloudGroupB_Git_Update/cloudGroupB/ceph_scr/faultRecovery.xls
