...
sleep 2s
ssh groupb@groupb13 "virsh start hdfsSlave8; virsh start hdfsSlave9; sleep 10s; virsh start hdfsMaster; sleep 5s; ssh hduser@hdfsMaster \"/usr/local/hadoop/sbin/start-dfs.sh; /usr/local/hadoop/sbin/start-yarn.sh; sleep 10s; /usr/local/hadoop/bin/hdfs dfsadmin -report; exit;\"; exit"
